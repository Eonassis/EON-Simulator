...
def Simulador(self, rate):
		...
		for count in range(1, NUM_OF_REQUESTS + 1):
			...
			sum_ht += holding_time
			#print("sum_ht>>>>>", sum_ht, holding_time)
			#print("1sum_ht>>>>>>", env.now , count, env.now, sum_time_up, holding_time)
			conta_requisicao_banda(bandwidth)
			conta_requisicao_classe(class_type)
			#if src == 9 or dst == 9:
			#	print(">>>>0",  src , dst)
				#input()
			#print("requisicao: ", count , "origem destino:", src, dst, "banda:", bandwidth)
			...
			if topology.has_edge(src, dst) and 'failed' in topology[src][dst] and topology[src][dst]['failed'] == True:
				...
				NumReqBlocked +=1
				#print("NumReqBlocked1", NumReqBlocked)
				...
				#	print(">>>>1",  src , dst)
					#input()
				##print("trafego bloqueado de nos com falhas na requisicao", count )
				#pass
			else:
				...
				for i in range(N_PATH):
					distance = int(Distance(paths[i]))  # Calcule a distância
					Fatormodulation = FatorModulation(distance)
					FreeSlots, FreeSlotsMatrix, INSlotsfreeMatrix = MaxFlow(paths[i])
				...
				if capacity >= bandwidth:
					for sublista in Datapaths:
						...
						for pathsAble in coluna_7:
							...
							##print("antes da alocacao: ", count , "caminho ", sublista[0] ,  "slots:",  MaxFlow(sublista[0]))
							...
							##print("depois da alocacao: ", count , "caminho ", sublista[0] , "slots:", MaxFlow(sublista[0])  )
							spectro = [DadosSOL[1], DadosSOL[2]]
							...
							processo = env.process(Desalocate(count,DadosSOL[0],spectro,holding_time, env.now))
							#print("requisicao chamou ", count,DadosSOL[0],spectro,holding_time, env.now)
							#print(count, DadosSOL[1], DadosSOL[2], DadosSOL[0])
							req_accepts.append([count, class_type, num_slots, DadosSOL[0], spectro, env.now , holding_time, 0, len(paths[i])-1, distance, bandwidth, processo])
							ativo[count] = 1
							#print(req_accepts)
							#input()
							#print("count1", count)
					...
					#print("Ativa count",count, class_type, num_slots, DadosSOL[0], spectro, env.now , holding_time, 0, len(paths[i])-1, distance, bandwidth , processo)
					#print(count, DadosSOL[1], DadosSOL[2], DadosSOL[0])
					#for u, v in list(topology.edges):
				...
				if flag == 0:
						...
						#contador += 1
						NumReqBlocked +=1
						#print("NumReqBlocked2", count, env.now)
						Vet_NumReqBlocked2x.append(count)
...
def FalhaNoLink(node1, node2):
	...
	if topology.has_edge(node1, node2):
		...
		for i in range(len(reqfalhou)):
			...
			process.interrupt()
			#sum_time_up += (env.now - reqfalhou[i][5])
...
def Desalocate(count, path, spectro, holding_time, inicio):
	...
	try:
		...
		for i in range(0, (len(path)-1)):
			...
			#print("spectro ===",spectro[0],spectro[1]+1)
		...
		if count not in desalocateReq1:
			...
			#print("1>>>>>>", env.now , count, inicio, sum_time_up, holding_time)
...
# Computa numero de requesicoes por banda
